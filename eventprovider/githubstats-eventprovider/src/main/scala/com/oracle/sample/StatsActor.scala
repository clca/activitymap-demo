...
final case class GitHubStat(state: String, numCommits: Long, numStars: Int, numEvents: Int)
final case class GitHubStats(stats: Seq[GitHubStat])

final case class GitHubCommit(state: String, numCommits: Long)
final case class GitHubCommits(commits: Seq[GitHubCommit])

object StatsActor {
  final case object GetStats
  final case class GetStat(state: String)
  final case object GetCommits
  final case class GetCommit(state: String)
...
class StatsActor extends Actor with ActorLogging {
  ...
  def receive: Receive = {
    ...
    case GetCommits =>
      stats = updateStats()
      sender() ! GitHubCommits(stats.map(e => GitHubCommit(e._1, e._2.numCommits)).toSeq)

    case GetCommit(state) =>
      val s = getStats(state)
      sender() ! GitHubCommit(s.state, s.numCommits)

    case _ =>
      log.error("uknown message in StatsActor")
  }
}
